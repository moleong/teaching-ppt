...
- 檔名：`{主題}-{用途}.html`
- **用途詞統一**：閱讀式產物檔名結尾用「**講義**」（如 `選修3第5章-原子核-講義.html`），簡報式產物檔名結尾用「**簡報**」（如 `選修3第5章-原子核-簡報.html`）
- **棄用** `閱讀版`、`簡報式` 作為檔名結尾（既有舊檔保留原名，不追溯改名；「簡報式／閱讀式」保留為形式名稱，僅用於選擇模板）
- 其他用途仍可用：`課堂練習`、`概念講解`、`例題`、`複習`
- 全部用中文檔名，方便老師快速辨識
